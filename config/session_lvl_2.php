<?php
    ...
    if(!empty($_SESSION['JWT_Dengue_Fever'])){
        ...
        $s_check = "SELECT count(*) as count_check FROM dhf_user WHERE dhf_user = '".$token->dhf_user."' AND dhf_level = '2';";  
        $r_check = $db_r4->prepare($s_check);
        $r_check->execute();
    ...
    if($now_page == "index_hi_ci.php" || $now_page == "manage_hici.php" || $now_page == "view_map_hici.php"){
        if($token->group_id != "1"){
